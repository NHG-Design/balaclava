...
type TooltipContentResolver =
  | TooltipContent
  | ((element: HTMLElement) => TooltipContent);
...
const VALID_POSITIONS = new Set<TooltipPosition>([
  "top",
  "bottom",
  "left",
  "right",
]);
const VALID_THEMES = new Set<TooltipTheme>([
  "system",
  "dark",
  "light",
  "custom",
]);
const CUSTOM_THEME_KEYS = new Set<keyof TooltipThemeTokens>([
  "bgColor",
  "textColor",
  "borderColor",
  "shadowColor",
...
const THEME_TOKENS: Readonly<
  Record<"dark" | "light", Readonly<TooltipThemeTokens>>
> = Object.freeze({
  dark: Object.freeze({
...
) as Window & Record<string, BalaclavaTooltipAPI | undefined>;
...
if (!rootWindow[API_NAME]?.version) {
  ...
  function getVisualConfig(): VisualTooltipConfig {
    return {
      ...
      arrowBorderColor:
        config.arrowBorderColor ?? "var(--balaclava-tooltip-border)",
  ...
  function exposeApi(): void {
    ...
    if ((window as unknown) !== (rootWindow as unknown)) {
      (
        window as object as Window &
          Record<string, BalaclavaTooltipAPI | undefined>
      )[API_NAME] = api;
    }
  }
    ...
    for (const [key, value] of Object.entries(userConfig)) {
      ...
      if (isConfigKey(key)) {
        ...
        hasCustomThemeOverride =
          hasCustomThemeOverride ||
          CUSTOM_THEME_KEYS.has(key as keyof TooltipThemeTokens);
      }
    }
  ...
  function attachTooltip(
    ...
    const doShow = (): void =>
      showTooltip(element, resolveContent(content, element), options);
  ...
  function scanElement(element: Element): void {
    ...
    const arrow =
      element.getAttribute("data-balaclava-tooltip-arrow") !== "false";
  ...
  function cleanupRemovedNode(node: Node): void {
    ...
    node
      .querySelectorAll?.("[data-balaclava-tooltip]")
      .forEach(cleanupAttachedElement);

    if (
      targetElement &&
      (node === targetElement || node.contains(targetElement))
    ) {
      hideTooltip();
    }
  }
  ...
  function renderTooltip(content: TooltipContent): void {
    ...
    tooltipEl.className = nextShowInstant
      ? getTooltipClassName()
      : `${getTooltipClassName()} is-entering`;
  ...
  function destroy(): void {
    ...
    const pageWindow = window as object as Window &
      Record<string, BalaclavaTooltipAPI | undefined>;

    if (
      (window as unknown) !== (rootWindow as unknown) &&
      pageWindow[API_NAME]?.version === VERSION
    ) {
      try {
  ...
  function normalizePosition(value: unknown): TooltipPosition {
    return typeof value === "string" &&
      VALID_POSITIONS.has(value as TooltipPosition)
  ...
  function normalizeOptionalTheme(value: unknown): TooltipTheme | null {
    ...
    return VALID_THEMES.has(theme as TooltipTheme)
      ? (theme as TooltipTheme)
      : null;
  }
