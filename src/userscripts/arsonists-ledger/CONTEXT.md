...
- **PayoutBasis**: `'min' | 'max'` (`engine.ts`), user-toggleable in Settings → Thresholds → "Payout basis". Selects whether `calcProfitPerNerve()`/`rankForScenario()` use `Scenario.payoutMin` (conservative) or `Scenario.payoutMax` (optimistic) as the payout input — see `effectivePayout()`. Persisted as `pyroLedger.v1.payoutBasis` (default `'min'`), and unlike the PPN bar position toggle, this one calls `resetScans()` since it changes the actual PPN figure/band per card, not just a CSS offset.
- **payoutMin / payoutMax**: Required fields on `Scenario` (`src/data/scenarios.ts`) — the observed payout range for the scenario's canonical recipe (see `actions`), derived from `scripts/analyze-arson-cookbooks.ts` matching cookbook CSV samples (`plans/arson/cookbooks/`) against each scenario's exact `place`/`stoke` recipe. Scenarios with fewer than 2 matching-recipe samples keep prior values, unrefreshed. Drives both the tooltip's payout range display and the `PayoutBasis` toggle.
...
- **Scenario data refresh**: independent of the price system — gated by `GM_xmlhttpRequest` availability, 24h TTL, keyed by `SCENARIOS_VERSION`, fetched from `https://balaclava.app/arsonists-ledger/scenarios.json`. TornPDA doesn't support this fetch (no `GM_xmlhttpRequest`), so mobile TornPDA users only ever see the *data* bundled into the script body at build time — they get neither this refresh nor (see below) an update mechanism, so a stale TornPDA install can lag arbitrarily far behind.
- **Distribution/updates**: `scripts/build-userscripts.mjs` generates a `.meta.js` alongside each `.user.js` (the lightweight file userscript managers are meant to poll for `@version` before re-downloading the full body) but deliberately does **not** set `@updateURL`/`@downloadURL` — updates are published and distributed through Greasyfork directly, not via a self-hosted update URL. Don't add those fields without checking with Yukio first; it was a deliberate call, not an oversight.
