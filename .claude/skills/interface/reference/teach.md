...
Every other interface reference reads these files before doing any work.
...
- **PRODUCT.md exists, DESIGN.md missing**: skip to Step 5 and offer to run `reference/document.md` for DESIGN.md.
...
If teach was invoked as a setup blocker by another command, such as loading `reference/craft.md` for a landing page, pause that task here. Complete teach, re-run the loader, then resume the original command with the freshly loaded context. For craft, resume into shape next; teach creates project context, but it is not a substitute for the task-specific shape interview and confirmed design brief.
...
Offer `reference/document.md` either way. Two paths:
...
If the user agrees, delegate to `reference/document.md` (it auto-detects scan vs seed). Load its reference and follow that flow.

If the user prefers to skip, mention they can run `reference/document.md` any time later.
...
**Critical: re-run the loader to refresh session context.** After writing PRODUCT.md, run `node .claude/skills/interface/scripts/load-context.mjs` one final time and let its full JSON output land in conversation. This ensures subsequent commands in this session use the freshly-written PRODUCT.md, not a stale earlier version.

If teach was invoked as a blocker by another interface reference (e.g. the user triggered a polish pass with no PRODUCT.md), resume that original task now with the fresh context.
