...
```
LOOP:
  node .claude/skills/interface/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
The live helper persists an append-only journal under `.interface/live/sessions/`. Browser checkpoints are advisory but durable; the journal is canonical. This is local durable recovery state, not project source.

Use these commands when the chat was interrupted, polling was missed, the helper restarted, or the browser reloaded:

```bash
node .claude/skills/interface/scripts/live-status.mjs
node .claude/skills/interface/scripts/live-resume.mjs --id SESSION_ID
node .claude/skills/interface/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .claude/skills/interface/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
- `scoped`: use `@scope ([data-interface-variant="N"])` rules.
- `astro-global-prefixed`: use explicit `[data-interface-variant="N"]` selector prefixes and the exact `styleTag` returned by the tool.
...
<style data-interface-css="SESSION_ID">
...
<div data-interface-variant="2" style="display: none">
  <!-- variant 2: full element replacement -->
</div>
<div data-interface-variant="3" style="display: none">
...
For `styleMode: "scoped"`, author every `:scope` rule with a descendant combinator. The `@scope` boundary is the **variant wrapper `<div data-interface-variant="N">`**, not the element you're designing. A bare `:scope { background: cream; }` styles the wrapper, not the inner replacement, so the cream lands on a `display: contents` shell while the actual element keeps page defaults. Always step in: `:scope > .card`, `:scope > section`, `:scope .hero-title`, etc. The fake test agent's CSS in `tests/live-e2e/agent.mjs` is a faithful template; every scoped rule starts `:scope > ...`.

**JSX / TSX target files.** Wrap `<style>` content in a template literal so the CSS `{` / `}` aren't parsed as JSX expressions, and use `className=` / `style={{…}}` on every variant element. Keep `data-interface-*` attributes as-is; they're plain strings:

```tsx
<style data-interface-css="SESSION_ID">{`
  @scope ([data-interface-variant="1"]) { ... }
  @scope ([data-interface-variant="2"]) { ... }
`}</style>
<div data-interface-variant="1">
  {/* variant 1 */}
</div>
...
The wrap script already gives you a single-rooted JSX wrapper: a `<div data-interface-variants="…">` outer element with the marker comments tucked inside. Drop the variants block above into the "Variants: insert below this line" comment and the source stays valid TSX.
...
**Freeform (`action` is `interface`) bias.** You did not load a sub-command reference, so you must **choose** signature axes yourself. Match the budget table: for a hero or large composition, that means **2–3 axes per variant**, not 1. Prefer knobs that sit on the dimensions where your three variants actually differ (if density varies, expose it as a `steps` knob; if color commitment varies, expose it as a `range`). A hero that ships with **0** params is almost always a mistake, not a judgment call. A hero with exactly **1** param is underweight unless the design is genuinely a fixed-point comparison. Start from the budget table, not from zero.
...
```html
<div data-interface-variant="1" data-interface-params='[
...
**Signature params per action.** For named sub-commands, read that action’s `reference/<action>.md` for one or two **MUST** params (e.g. `layout` → `density`). Those are non-negotiable when the design can express them. **Freeform has no file-level MUST**; the **Freeform (`interface`) bias** in this section is the stand-in. If the user’s action is both stylized and sub-command (e.g. `colorize`), the sub-command’s MUST list takes precedence for its axes; still respect the **Hard cap** and add no redundant duplicate knobs.
...
```html
<!-- interface-param-values SESSION_ID: {"color-amount":0.7,"density":"packed"} -->
```
...
```bash
node .claude/skills/interface/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .claude/skills/interface/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
1. Manually write the wrapper scaffold into the **served** file (the one the browser actually loaded). Use the same structure `live-wrap.mjs` produces; `<!-- interface-variants-start ID --><div data-interface-variants="ID" data-interface-variant-count="3" style="display: contents">…</div><!-- end -->`.
...
3. **Bake in parameter values while rewriting selectors.** For `@scope ([data-interface-variant="N"])` wrappers: retarget to real, semantic classes on the accepted HTML (`.why-visual--v2 .v2-label { … }`). For `:scope[data-p-<id>="VALUE"]` selectors: keep only the branch matching the chosen value from the param-values comment; drop the others (they're dead after accept). For `var(--p-<id>, DEFAULT)` in the CSS: either substitute the literal value, or if the param is still useful as a knob going forward, leave the var and update its initial declaration to the chosen value.
4. **Unwrap the accepted content.** Delete the `<div data-interface-variant="N" style="display: contents">` that wraps it. Drop `data-interface-params` and any `data-p-*` attributes from it; those are live-mode plumbing, not source.
5. **Delete the inline `<style>` block, the `<!-- interface-param-values -->` comment if present, and both `<!-- interface-carbonize-start/end -->` markers.** Also drop any `@scope` rules for variants other than the accepted one; those are dead code now.
...
```bash
node .claude/skills/interface/scripts/live-server.mjs stop
```

Stops the HTTP server and runs `live-inject.mjs --remove` to strip `localhost:…/live.js` from the HTML entry. To stop the server but keep the inject tag (for a quick restart), use `stop --keep-inject`. `.interface/live/config.json` persists as project config for future sessions.

Then:
- Remove any leftover variant wrappers (search for `interface-variants-start` markers).
- Remove any leftover carbonize blocks (search for `interface-carbonize-start` markers).

## First-time setup (config missing or invalid)

If `live.mjs` outputs `{ ok: false, error: "config_missing" | "config_invalid", path }`, write the live config at the reported path. By default this is `.interface/live/config.json`.
...
```bash
node .claude/skills/interface/scripts/detect-csp.mjs
```
...
- **`null`**: no CSP; skip to writing `.interface/live/config.json` with `cspChecked: true`.
...
```ts
// Dev-only allowance so interface live mode can load. Guarded by NODE_ENV.
const __interfaceLiveDev =
  process.env.NODE_ENV === "development" ? ["http://localhost:8400"] : [];
```

**Append `...__interfaceLiveDev` to the script-src and connect-src directive arrays.** Per-framework specifics:
...
Idempotency: if `__interfaceLiveDev` already exists in the file, the patch is already applied; skip asking and just mark `cspChecked: true`.
...
```ts
// Dev-only allowance so interface live mode can load.
const __interfaceLiveDev =
  process.env.NODE_ENV === "development" ? " http://localhost:8400" : "";
```

Then in the CSP value string:
- `script-src 'self' 'unsafe-inline'` → `` `script-src 'self' 'unsafe-inline'${__interfaceLiveDev}` ``
- `connect-src 'self'` → `` `connect-src 'self'${__interfaceLiveDev}` ``
...
If a user says "no" to the CSP patch at setup time and later complains that live doesn't work: their dev CSP blocks `http://localhost:8400`. Fix: delete `cspChecked` from `.interface/live/config.json` and re-run `live.mjs`: setup will ask again.
