...
// @updateURL   https://raw.githubusercontent.com/NHG-Design/balaclava/main/dist/pyromaniacs-ledger.meta.js
// @downloadURL https://raw.githubusercontent.com/NHG-Design/balaclava/main/dist/pyromaniacs-ledger.user.js
...
(() => {
  ...
  function strategyNeedsFlamethrower(s) {
  ...
  function buildPrimaryBlock(ranked, statsOnly = false) {
  ...
  function buildAltRow(ranked) {
    ...
    const ftTag = strategyNeedsFlamethrower(ranked.strategy) ? " \xB7 FT" : "";
  ...
  function buildTooltipContent(allRanked, statsOnly = false) {
...
`;
  ...
  function populateStrategyIndex(strategies) {
    strategyIndex.clear();
  ...
  function scheduleStrategyRefresh() {
    ...
    if (now - ts < STRATEGIES_TTL_MS) {
      try {
        const cached = JSON.parse(store_get(KEY_STRATEGIES_CACHE, ""));
  ...
  function buildTooltipContentWithStyles(allRanked, statsOnly = false) {
